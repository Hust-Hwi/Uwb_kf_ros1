...
// ---------------- 新增：协议定义的 CRC16 校验函数 [cite: 32, 292, 293] ----------------
static uint16_t term_crc16(const uint8_t *data, uint16_t len) {
...
public:
    UwbApiClient(const std::string& base_url,
        ...
        : base_url_(base_url),
          ...
          goal_stale_timeout_sec_(std::max(1.0, 3.0 * goal_publish_interval_sec)),
          is_running_(true),
...
private:
    ...
    void networkWorkerLoop() {
        while (true) {
            ...
            if (getLatestGoal(goal)) {
                double goal_age = (ros::Time::now() - goal.stamp).toSec();
                if (goal_age > goal_stale_timeout_sec_) {
                    ROS_WARN_THROTTLE(1.0, "UWB导航目标 %.2fs 未更新，暂停下发新目标。", goal_age);
                    clearLatestGoal();
                    continue;
                } else {
                    last_goal_process_time_ = ros::Time::now();
    ...
    bool waitUntilReadyToProcess() {
        while (true) {
            ...
            if (last_goal_process_time_.toSec() != 0.0) {
                wait_sec = goal_publish_interval_sec_ - (now - last_goal_process_time_).toSec();
            }
    ...
    void processGoal(const BodyGoal& goal, const RobotPose& robot_pose) {
        ...
        double target_world_yaw = robot_pose.yaw + goal.body_yaw; 
        ...
        if (has_published_goal_) {
            ...
            double dt_goal = (ros::Time::now() - last_goal_publish_time_).toSec();
            if (dt_goal > 0.0) {
    ...
    double max_fov_deg_;
    double goal_stale_timeout_sec_;
...
private:
    ...
    const double GOAL_DISTANCE_THRESHOLD = 0.3; // 距离上一次发布的坐标超过此阈值时更新
    const double ABSOLUTE_TRACKING_DISTANCE = 1; // 目标绝对距离大于此阈值时触发追踪
    const double GOAL_YAW_THRESHOLD_DEG = 3.0;   // 角度目标变化超过该值才重新下发
    const double GOAL_PUBLISH_INTERVAL_SEC = 0.333333333;
    const double MAX_NAV_SPEED_M_S = 2.0;
    const double MAX_FOV_DEG = 30.0;

    struct NavGoalSample {
        ros::Time stamp;
        double x;
        double y;
        double yaw;
    ...
    std::deque<NavGoalSample> nav_goal_history_;
    ...
    bool send_init_ranging_request() {
        static const uint8_t init_ranging_frame[] = {
            0xA5, 0x5A, 0x08, 0x00, 0x0F, 0x00, 0x58, 0x6A
        };

        ssize_t written = write(serial_fd_, init_ranging_frame, sizeof(init_ranging_frame));
        if (written != static_cast<ssize_t>(sizeof(init_ranging_frame))) {
            ROS_WARN("发送 Init 外部触发测距请求失败，written=%zd", written);
            return false;
        }
    ...
    bool pulse_serial_reset_lines() {
        int status = 0;
        if (ioctl(serial_fd_, TIOCMGET, &status) != 0) {
            ROS_WARN("无法读取串口控制线状态，跳过 DTR/RTS 复位脉冲");
            return false;
        }

        status &= ~(TIOCM_DTR | TIOCM_RTS);
        if (ioctl(serial_fd_, TIOCMSET, &status) != 0) {
            ROS_WARN("无法拉低 DTR/RTS，跳过软件复位脉冲");
            return false;
        }
        ...
        status |= (TIOCM_DTR | TIOCM_RTS);
        if (ioctl(serial_fd_, TIOCMSET, &status) != 0) {
            ROS_WARN("无法拉高 DTR/RTS，软件复位脉冲不完整");
            return false;
        }
    ...
    void prime_uwb_after_open() {
        tcflush(serial_fd_, TCIFLUSH);

        pulse_serial_reset_lines();
        std::this_thread::sleep_for(std::chrono::milliseconds(800));

        for (int attempt = 0; attempt < 5; ++attempt) {
            if (send_init_ranging_request()) {
                ROS_INFO("已发送 Init 测距启动请求 0x0F（第 %d 次）", attempt + 1);
            }
    ...
    void serial_read_loop() {
        ...
        const double MAX_SPEED_THETA_RAD_S = 1;        // 角度最大变换速度
        const double MAX_SPEED_XY_M_S = 3.0;           // 二维平面最大跳变速度
        const double MIN_RADIUS_FOR_ANGLE_CHECK = 1.0; // 小于 1.0m 时豁免角度跳变检查
        ...
        const double NAV_BLOCK_DURATION = 0.026;  //拒绝后导航静默期
        ...
        while (ros::ok()) {
            ...
            if (n > 0) {
                ...
                if (header_idx != -1) {
                    ...
                    // 至少包含 Header(2) + PayloadLen(2)
                    if (buf_len >= 4) {
                        // 解析包长 (小端模式) [cite: 34]
                        uint16_t packet_length = buf[2] | (buf[3] << 8); 
                        
                        if (packet_length < 8 || packet_length > sizeof(buf)) {
                            ROS_WARN_THROTTLE(1.0, "收到非法包长 %u，丢弃当前头部", packet_length);
                        ...
                        if (buf_len >= packet_length) {
                            // CRC16 校验，包含除末尾 2 字节外所有数据 [cite: 34]
                            uint16_t crc_calc = term_crc16(buf, packet_length - 2);
                            uint16_t crc_recv = buf[packet_length - 2] | (buf[packet_length - 1] << 8);
                            
                            if (crc_calc != crc_recv) {
                                ROS_WARN_THROTTLE(1.0, "CRC 校验失败 (calc:0x%X recv:0x%X)，丢弃该帧", crc_calc, crc_recv);
                                memmove(buf, buf + 2, buf_len - 2);
                                buf_len -= 2;
                                continue;
                            }
                            ...
                            // 定位帧判定 0x01，且满足该帧长度 [cite: 15, 38, 66]
                            if (command == 0x01 && packet_length >= 40) {
                                
                                // 解析距离 (cm)，小端提取 [cite: 76]
                                uint16_t distance_cm = buf[12] | (buf[13] << 8);
                                
                                // 解析 Resp 测量 Init 的水平角 (单位 0.01 度) [cite: 78]
                                int32_t angle_resp_azimuth = buf[16] | (buf[17] << 8) | (buf[18] << 16) | (buf[19] << 24);
                                
                                // 验证角度有效性 (0x80000000 为无效值) [cite: 78]
                                if (static_cast<uint32_t>(angle_resp_azimuth) != 0x80000000) {
                                    auto current_time = ros::Time::now();
                                    
                                    double azimuth_deg = angle_resp_azimuth * 0.01;

                                    geometry_msgs::PointStamped msg_raw_da;
                                    msg_raw_da.header.stamp = current_time;
                                    msg_raw_da.header.frame_id = "uwb_base_link";
                                    msg_raw_da.point.x = distance_cm; 
                                    msg_raw_da.point.y = azimuth_deg;  
                                    raw_dist_angle_pub_.publish(msg_raw_da);

                                    double raw_r = distance_cm / 100.0; // cm 转 m
                                    double raw_theta = azimuth_deg * M_PI / 180.0;
                                    ...
                                    if (is_valid) {
                                        ...
                                        kf.update_R_matrix(raw_x, raw_y, 0.1, 7.0, 20.0);
                                        ...
                                        if (is_nav_blocked) {
                                            ROS_WARN_THROTTLE(1.0, "UWB信号抖动/被遮挡(存在近期丢弃点)，暂停所有目标点更新。");
                                            nav_goal_history_.clear();
                                            if (api_client_) {
                                                api_client_->clearLatestGoal();
                                            }
                                        } else {
                                            double body_yaw = nav_target_yaw;
                                            nav_goal_history_.push_back(NavGoalSample{current_time, nav_target_x, nav_target_y, body_yaw});
                                            while (!nav_goal_history_.empty() && (current_time - nav_goal_history_.front().stamp).toSec() > NAV_BLOCK_DURATION) {
                                                nav_goal_history_.pop_front();
                                            }
                                            ...
                                            for (const auto& sample : nav_goal_history_) {
                                                avg_x += sample.x;
                                                avg_y += sample.y;
                                            ...
                                            if (!nav_goal_history_.empty()) {
                                                const double inv_count = 1.0 / static_cast<double>(nav_goal_history_.size());
                                                avg_x *= inv_count;
                                                avg_y *= inv_count;
                                            }

                                            double avg_yaw = std::atan2(sum_sin_yaw, sum_cos_yaw);

                                            if (api_client_) {
                                                api_client_->updateLatestGoal(avg_x, avg_y, avg_yaw, d);
                                            }
                                        }
                                    }
                                } else {
                                    ROS_WARN_THROTTLE(1.0, "当前收到的 UWB 角度值无效(0x80000000)，跳过当前帧处理");
                                }
                            }
                            // 丢弃已完整解析的一帧
                            memmove(buf, buf + packet_length, buf_len - packet_length); 
                            buf_len -= packet_length;
                        }
                    }
                } else if (buf_len > 200) {
                    // 没有找到有效 Header，防止内存堆积
                    buf_len = 0; 
                }
            }
        }
